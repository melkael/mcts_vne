...
class MCF_vne(object):
    ...
    def undirected_to_directed_normalized(self, g):
        ...
        for e in edges:
            ...
            g.add_edge(i, i_prime, BW_max=GRB.INFINITY, BW_used=0)
            g.add_edge(j, i_prime, BW_max=GRB.INFINITY, BW_used=0)
            g.add_edge(j_prime, i, BW_max=GRB.INFINITY, BW_used=0)
            g.add_edge(j_prime, j, BW_max=GRB.INFINITY, BW_used=0)
...
vnr_returned = None
vnr = VNR(n_nodes_min=4, n_nodes_max=4)
result, vnr_returned = agent.VNoM(vnr)

mcf = MCF_vne(vnr_returned, sn)
a, flow_returned = mcf.solve()

pp = pprint.PrettyPrinter(indent=4)
pp.pprint(flow_returned)
